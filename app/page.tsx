    ...
    <>
      ...
      <section className="relative h-[100svh] min-h-[560px] w-full overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-[12vh] flex flex-col items-center gap-5 text-paper px-6">
          ...
          <Link
            href="/account"
            className="text-paper text-xs font-display uppercase tracking-widest2 underline underline-offset-4 hover:opacity-70 transition-opacity rise rise-2"
          >
      ...
      <section className="mx-auto max-w-7xl px-4 md:px-8 py-16">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {tiles.map((c, i) => (
            <Reveal key={c.id} delay={(Math.min(i, 3) as 0 | 1 | 2 | 3)}>
              <Link
                href={`/shop?cat=${c.slug}`}
                className="group/card relative block aspect-[3/4] overflow-hidden bg-offwhite"
              >
                <Img
                  src={c.imageUrl}
                  alt={c.name}
          ...
          {!loading && tiles.length === 0 && (
            <p className="col-span-full text-center text-stone py-12">No categories yet.</p>
          )}
        </div>
      </section>
